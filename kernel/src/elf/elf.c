...
struct sched_task *elfLoad(const char *path, int argc, char **argv, bool driver)
{
    ...
    Elf64_Ehdr *elf = pmmPage();   // allocate the elf header
    ...
    Elf64_Phdr *phdr = pmmPage();                   // buffer to store information about the current program header
    ...
    pmmDeallocate(phdr); // clean up

    char *cwd = pmmPage(strlen(path));
